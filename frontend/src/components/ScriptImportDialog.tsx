...
export function ScriptImportDialog({ open, busy, preview, characters, close, selectFile, pasteText, updatePreview, apply }: ScriptImportDialogProps) {
  ...
  return <div className="modal-backdrop" onClick={close}><div className="modal wide script-import-modal" role="dialog" aria-modal="true" aria-labelledby="script-import-title" onClick={(event) => event.stopPropagation()}>
    ...
    {rulesOpen && <section className="script-import-rules" aria-label="文本解析规则">
      <label><span>对话分隔符<small>识别角色与正文</small></span><Select value={rules.dialogueSeparator} onChange={(value) => updateRule('dialogueSeparator', value as ScriptImportRules['dialogueSeparator'])}><option value="auto">自动（冒号或 Tab）</option><option value="colon">仅冒号 ：</option><option value="tab">仅 Tab 制表符</option></Select></label>
      <label><span>表情标记<small>位于角色名或正文开头</small></span><Select value={rules.expressionSyntax} onChange={(value) => updateRule('expressionSyntax', value as ScriptImportRules['expressionSyntax'])}><option value="auto">自动识别</option><option value="brackets">方括号 [微笑]</option><option value="parentheses">圆括号（微笑）</option><option value="pipe">竖线 |微笑</option><option value="none">不识别表情</option></Select></label>
      <label><span>角色匹配<small>主名称与固定显示名</small></span><Select value={rules.characterMatching} onChange={(value) => updateRule('characterMatching', value as ScriptImportRules['characterMatching'])}><option value="smart">智能匹配</option><option value="exact">仅精确匹配</option></Select></label>
      <label><span>未知角色<small>无法匹配项目角色时</small></span><Select value={rules.unknownCharacter} onChange={(value) => updateRule('unknownCharacter', value as ScriptImportRules['unknownCharacter'])}><option value="keep">保留原角色名</option><option value="narration">整行转为旁白</option></Select></label>
    ...
    {!preview ? <div className="import-picker"><FileText /><strong>TXT、Markdown、Hikari JSON 或剪贴板文本</strong><span>Python 会结合项目角色和上述规则生成 Block，确认前不会修改项目。</span><div className="import-picker-actions"><button className="button primary" disabled={busy} onClick={() => selectFile(rules)}>{busy ? <LoaderCircle className="spin" /> : <FileUp />}{busy ? '正在解析' : '选择剧本文件'}</button><button className="button ghost" disabled={busy} onClick={() => pasteText(rules)}><ClipboardPaste />粘贴文本</button></div></div> : <>
      ...
      {(unmatchedGroups.length > 0 || expressionGroups.length > 0) && <section className="script-import-resolver" aria-label="批量映射">
        ...
        <div className="script-resolver-grid">
          {unmatchedGroups.map(([rawSpeaker, group]) => <label key={`speaker-${rawSpeaker}`}><span><UserRound /><span><strong>{rawSpeaker}</strong><small>{group.length} 行未知角色</small></span></span><Select aria-label={`批量映射角色 ${rawSpeaker}`} defaultValue="" onChange={(value) => value && mapCharacters(group.map((match) => match.blockId), value)}><option value="" disabled>映射到项目角色…</option>{characters.map((character) => <option key={character.id} value={character.id}>{character.name}</option>)}</Select></label>)}
          {expressionGroups.map((group) => { const first = group[0]; const character = characterById.get(first.characterId!); if (!character) return null; const source = first.rawExpression ?? first.expression; return <label key={`expression-${first.characterId}-${source}`}><span><Smile /><span><strong>{source}</strong><small>{character.name} · {group.length} 行异常表情</small></span></span><Select aria-label={`批量映射表情 ${character.name} ${source}`} defaultValue="" onChange={(value) => value && mapExpressions(group.map((match) => match.blockId), value)}><option value="" disabled>替换为真实表情…</option>{character.expressions.map((expression) => <option key={expression} value={expression}>{expression}</option>)}</Select></label>; })}
        </div>
      </section>}
        ...
        return <article className={needsReview ? 'needs-review script-row-editor' : 'script-row-editor'} key={block.id}>
          <span>{index + 1}</span><strong>{block.type}</strong><div className="script-row-content">
            ...
            {block.type === 'dialogue' && <div className="script-row-controls">
              <label><span>角色</span><Select aria-label={`第 ${index + 1} 行角色`} value={match?.characterId ?? ''} onChange={(value) => mapCharacters([block.id], value)}><option value="" disabled>{block.speaker || '选择角色'}</option>{characters.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select></label>
              <label><span>表情</span><Select aria-label={`第 ${index + 1} 行表情`} disabled={!character || character.expressions.length === 0} value={expressionNeedsReview(match) ? '__review__' : block.expression ?? ''} onChange={(value) => mapExpressions([block.id], value)}>{expressionNeedsReview(match) && <option value="__review__" disabled>请选择真实表情</option>}{character && character.expressions.length > 0 ? character.expressions.map((expression) => <option key={expression} value={expression}>{expression}</option>) : <option value="">{character ? '角色未配置表情' : '先匹配角色'}</option>}</Select></label>
