...
export function StageTimelineWorkspace({ project, selectedBlock, commit, locateBlock, notify }: Props) {
  ...
  return <div className="stage-timeline-workspace" data-testid="stage-timeline-workspace" ref={workspaceRef}>
    ...
    <div className="stage-timeline-upper">
      <section className="stage-timeline-preview"><Preview project={project} editorIndex={editorIndex} timelinePreview={evaluateTimelineAtTime(timeline, playhead)} onEditorLocationChange={(_, index) => locateBlock(index)} onStageCharacterMove={moveStageCharacter} onResolutionChange={(resolution) => commit((current) => ({ ...current, meta: { ...current.meta, resolution } }), '修改预览分辨率')} /></section>
