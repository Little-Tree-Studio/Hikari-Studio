      ...
      <aside className="timeline-clip-inspector">
        ...
        {selected ? <div className="timeline-inspector-fields">
          ...
          {selectedKeyframe && <section className="timeline-keyframe-inspector">
            ...
            <div className="timeline-keyframe-grid"><label>时间<input aria-label="关键帧时间" type="number" min="0" max={selected.clip.duration} step=".01" value={selectedKeyframe.time} onChange={(event) => editKeyframe({ time: Number(event.target.value) })} /></label><label>属性<select aria-label="关键帧属性" value={selectedKeyframe.property} onChange={(event) => editKeyframe({ property: event.target.value })}>{PROPERTY_OPTIONS[selected.track.kind].map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><label>值<input aria-label="关键帧值" type="number" step=".01" value={Number(selectedKeyframe.value)} onChange={(event) => editKeyframe({ value: Number(event.target.value) })} /></label><label>缓动<select aria-label="关键帧缓动" value={selectedKeyframe.easing} onChange={(event) => { const easing = event.target.value as TimelineEasing; editKeyframe({ easing, bezier: easing === 'cubicBezier' ? easingBezier(easing, selectedKeyframe.bezier) : undefined }, '调整关键帧缓动'); }}>{Object.entries(EASING_NAMES).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label></div>
