...
import {
  ArrowUpDown, AudioLines, CheckCircle2, CircleAlert, Clock3, FileAudio, FolderOpen, Gauge,
...
export function AudioManager({ project, category, setCategory, commit, notify, requestConfirm, activate }: AudioManagerProps) {
  ...
  const audioAssets = useMemo(() => project.assets.filter((asset) => asset.kind === 'audio'), [project.assets]);
  const filtered = useMemo(() => {
    const list = audioAssets.filter((asset) => audioCategoryOf(asset) === category
    ...
    const refs = (asset: Asset) => references.references[asset.id]?.length ?? 0;
    return [...list].sort((a, b) => sortKey === 'name' ? a.name.localeCompare(b.name, 'zh-Hans-CN')
      : sortKey === 'size' ? (b.size ?? 0) - (a.size ?? 0)
        : sortKey === 'duration' ? (b.duration ?? 0) - (a.duration ?? 0)
          : refs(b) - refs(a));
  }, [audioAssets, category, voiceCharacterId, query, sortKey, references]);
