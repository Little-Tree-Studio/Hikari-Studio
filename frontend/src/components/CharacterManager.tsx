  ...
  return <div className="character-manager">
    ...
    <div className="character-workspace">
      ...
      {selected && <aside className="character-property-panel"><div className="property-panel-title"><div><SlidersHorizontal /><strong>角色详情</strong></div><button className="icon-button" title="删除角色" onClick={() => void removeCharacter()}><Trash2 /></button></div><div className="character-property-scroll">
        ...
        <section><div className="property-section-heading"><h2>显示名方案</h2><button className="icon-button" title="新增显示名方案" onClick={() => void addScheme()}><Plus /></button></div><div className="display-name-schemes">{(selected.displayNameSchemes ?? []).map((scheme) => <article key={scheme.id}><div><input aria-label="方案名称" value={scheme.name} onChange={(event) => patchScheme(scheme.id, { name: event.target.value })} /><button className="icon-button" title="删除显示名方案" disabled={Boolean(schemeReferenceCount(scheme.id))} onClick={() => void removeScheme(scheme)}><Trash2 /></button></div><select aria-label="显示名来源" value={scheme.kind} onChange={(event) => { const kind = event.target.value as DisplayNameSchemeKind; patchScheme(scheme.id, { kind, value: kind === 'fixed' ? scheme.name : '' }); }}><option value="fixed">固定文字</option><option value="variable">剧情变量</option><option value="attribute">角色属性</option></select>{scheme.kind === 'fixed' ? <input aria-label="固定显示文字" value={scheme.value} placeholder="玩家看到的名字" onChange={(event) => patchScheme(scheme.id, { value: event.target.value })} /> : <select aria-label={scheme.kind === 'variable' ? '剧情变量' : '角色属性'} value={scheme.value} onChange={(event) => patchScheme(scheme.id, { value: event.target.value })}><option value="">请选择</option>{(scheme.kind === 'variable' ? textVariables : Object.keys(selected.attributes ?? {})).map((name) => <option key={name} value={name}>{name}</option>)}</select>}<small>{schemeReferenceCount(scheme.id)} 条对白引用{scheme.kind !== 'fixed' && ' · 空值时回退主名称'}</small></article>)}{!selected.displayNameSchemes?.length && <div className="property-empty">暂无方案，对白默认显示角色主名称</div>}</div></section>
        <section><h2>立绘尺寸</h2><label className="checkbox-field"><input type="checkbox" checked={selected.keepAspectRatio ?? true} onChange={(event) => patchCharacter({ keepAspectRatio: event.target.checked }, '设置保持原始比例')} />保持原始比例</label><div className="dimension-grid">{(['width', 'height'] as const).map((axis) => { const value = axis === 'width' ? selected.portraitWidth : selected.portraitHeight; return <label key={axis}>{axis === 'width' ? '宽度' : '高度'}<div><input type="number" min="0" value={value?.value ?? ''} placeholder="自动" onChange={(event) => setDimension(axis, event.target.value, value?.unit ?? 'px')} /><select value={value?.unit ?? 'px'} onChange={(event) => setDimension(axis, value?.value === undefined ? '' : String(value.value), event.target.value as 'px' | '%')}><option value="px">px</option><option value="%">%</option></select></div></label>; })}</div><button className="button ghost full" onClick={() => patchCharacter({ portraitWidth: undefined, portraitHeight: undefined }, '自适应立绘尺寸')}>自适应</button><div className="property-two-column"><label>默认站位<select value={selected.defaultPosition ?? 'center'} onChange={(event) => patchCharacter({ defaultPosition: event.target.value as CharacterPosition }, '修改默认站位')}><option value="farLeft">最左</option><option value="left">左侧</option><option value="center">中央</option><option value="right">右侧</option><option value="farRight">最右</option></select></label><label>默认层级<input type="number" value={selected.defaultLayer ?? 0} onChange={(event) => patchCharacter({ defaultLayer: Number(event.target.value) }, '修改默认层级')} /></label></div></section>
        <section><h2>当前表情资源</h2><label>立绘素材<select value={portraitId ?? ''} onChange={(event) => setPortrait(event.target.value)}><option value="">未配置</option>{imageAssets.map((asset) => <option key={asset.id} value={asset.id}>{asset.name}</option>)}</select></label><div className={`portrait-health-row ${portraitHealth.tone}`}>{portraitHealth.tone === 'ready' ? <CheckCircle2 /> : portraitHealth.tone === 'checking' ? <LoaderCircle className="spinning" /> : portraitHealth.tone === 'empty' ? <FileImage /> : <AlertTriangle />}<span><strong>{portraitHealth.label}</strong><small>{portraitHealth.detail}</small></span>{portraitHealth.canRelink && <button className="icon-button" title="重新定位立绘素材" disabled={relinkingAssetId === portraitId} onClick={() => void relinkPortrait()}><LocateFixed /></button>}</div><div className="resource-reference"><span>{portraitAsset?.path ?? portraitId ?? '没有资源路径'}</span><small>{dialogueReferences.length} 条对白使用此表情</small></div><div className="crop-editor"><div className="property-section-heading"><h2><Crop />缩略图裁剪</h2><select title="套用其他表情裁剪" value="" onChange={(event) => event.target.value && copyCrop(event.target.value)}><option value="">套用其他表情</option>{selected.expressions.filter((expression) => expression !== selectedExpression).map((expression) => <option key={expression} value={expression}>{expression}</option>)}</select></div><label>水平位置<input type="range" min="-50" max="50" value={currentCrop.x} onChange={(event) => patchCrop({ x: Number(event.target.value) })} /></label><label>垂直位置<input type="range" min="-50" max="50" value={currentCrop.y} onChange={(event) => patchCrop({ y: Number(event.target.value) })} /></label><label>头像缩放 <span>{Math.round(currentCrop.zoom * 100)}%</span><input type="range" min="1" max="3" step=".05" value={currentCrop.zoom} onChange={(event) => patchCrop({ zoom: Number(event.target.value) })} /></label></div><button className="button danger full" disabled={selected.expressions.length <= 1 || Boolean(expressionReferenceCount(selectedExpression))} onClick={() => void removeSelectedExpressions()}><Trash2 />删除所选表情</button></section>
        <section><div className="property-section-heading"><h2><Layers3 />图形覆盖</h2><button className="icon-button" title="新增覆盖图层" onClick={addOverlay}><Plus /></button></div><div className="character-overlays">{(selected.overlays ?? []).map((overlay) => { const issue = overlayIssue(overlay.assetId); return <article className={issue && overlay.assetId ? 'asset-error' : ''} key={overlay.id}><div><input aria-label="覆盖图层名称" value={overlay.name} onChange={(event) => patchOverlay(overlay.id, { name: event.target.value })} /><button className="icon-button" title="删除覆盖图层" onClick={() => patchCharacter({ overlays: selected.overlays?.filter((item) => item.id !== overlay.id) }, '删除图形覆盖')}><X /></button></div><select aria-label="覆盖图层素材" value={overlay.assetId ?? ''} onChange={(event) => patchOverlay(overlay.id, { assetId: event.target.value || undefined })}><option value="">选择覆盖素材</option>{imageAssets.map((asset) => <option key={asset.id} value={asset.id}>{asset.name}</option>)}</select><div className={`overlay-health ${issue ? 'error' : 'ready'}`}>{issue ? <AlertTriangle /> : <CheckCircle2 />}<span>{issue ?? '素材已就绪'}</span>{overlay.assetId && <button className="icon-button" title="重新定位覆盖图层" disabled={relinkingAssetId === overlay.assetId} onClick={() => void relinkOverlay(overlay.assetId!, overlay.name)}>{relinkingAssetId === overlay.assetId ? <LoaderCircle className="spinning" /> : <LocateFixed />}</button>}</div><div className="property-two-column"><label>透明度<input type="number" min="0" max="1" step=".1" value={overlay.opacity} onChange={(event) => patchOverlay(overlay.id, { opacity: Number(event.target.value) })} /></label><label>层级<input type="number" value={overlay.layer} onChange={(event) => patchOverlay(overlay.id, { layer: Number(event.target.value) })} /></label></div><label className="checkbox-field"><input type="checkbox" checked={overlay.overrideSize ?? false} onChange={(event) => patchOverlay(overlay.id, { overrideSize: event.target.checked })} />覆盖默认尺寸</label>{overlay.overrideSize && <div className="dimension-grid"><label>宽度<input type="number" value={overlay.width?.value ?? ''} onChange={(event) => patchOverlay(overlay.id, { width: { value: event.target.value === '' ? undefined : Number(event.target.value), unit: overlay.width?.unit ?? 'px' } })} /></label><label>高度<input type="number" value={overlay.height?.value ?? ''} onChange={(event) => patchOverlay(overlay.id, { height: { value: event.target.value === '' ? undefined : Number(event.target.value), unit: overlay.height?.unit ?? 'px' } })} /></label></div>}</article>; })}{!selected.overlays?.length && <div className="property-empty">暂无覆盖图层</div>}</div></section>
