...
export function SceneManager({ project, commit, notify, requestText, requestConfirm, activate }: Props) {
  ...
  const chooseImages = async (mode: 'scenes' | 'layers') => {
    ...
    if (!window.__SLIDE_DESKTOP__) return fileInputRef.current?.click();
  ...
  const backgroundLayer = selected?.layers.at(-1);
  const backgroundAsset = backgroundLayer ? project.assets.find((asset) => asset.id === backgroundLayer.assetId) : undefined;
  const workspaceBackgroundUri = backgroundLayer && !layerIssue(backgroundLayer) ? backgroundAsset?.uri : undefined;
  ...
  return <div className="scene-manager">
    ...
    <div className="scene-manager-workspace">
      ...
      {selected ? <main className={`scene-stage-panel ${workspaceBackgroundUri ? 'has-scene-background' : ''} ${dropTarget === 'stage' ? 'drop-active' : ''}`} onDragOver={(event) => { event.preventDefault(); setDropTarget('stage'); }} onDragLeave={() => setDropTarget(null)} onDrop={(event) => { event.preventDefault(); setDropTarget(null); if (event.dataTransfer.files.length) void handleFiles(event.dataTransfer.files, 'layers'); }}>
        {workspaceBackgroundUri && <img className="scene-workspace-background" src={workspaceBackgroundUri} alt="" draggable={false} onError={() => backgroundAsset && setFailedAssetIds((current) => new Set(current).add(backgroundAsset.id))} />}
        ...
        <div className="scene-camera-controls"><label>镜头 X<Slider ariaLabel="镜头 X" min={-30} max={30} value={camera.x} onChange={(value) => setCamera({ ...camera, x: value })} /></label><label>镜头 Y<Slider ariaLabel="镜头 Y" min={-30} max={30} value={camera.y} onChange={(value) => setCamera({ ...camera, y: value })} /></label><p>预览镜头仅用于检查视差，不写入剧本 Camera Block。</p></div>
