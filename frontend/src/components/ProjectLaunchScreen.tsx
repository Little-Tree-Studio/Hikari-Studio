...
import { ArrowLeft, ArrowRight, Check, ChevronDown, Clock3, FilePlus2, FolderOpen, Gamepad2, LayoutTemplate, LoaderCircle, Minus, Monitor, Pin, PinOff, RefreshCw, Square, X } from 'lucide-react';
import { callWindow, listRecentProjects, selectProjectLocation, setProjectCreationWindowMode, setProjectPinned } from '../api';
...
function WindowButtons({ onExit, compact = false }: { onExit: () => void; compact?: boolean }) {
  return <div className="launch-window-controls titlebar-no-drag">
    <button title="最小化" onClick={() => void callWindow('minimize_window')}><Minus /></button>
    {!compact && <button title="最大化或还原" onClick={() => void callWindow('toggle_maximize')}><Square /></button>}
    <button className="close" title="关闭" onClick={onExit}><X /></button>
  </div>;
}

export function ProjectLaunchScreen({ startInWizard = false, onOpen, onOpenRecent, onCreate, onCreated, onExit, ready = true }: Props) {
  ...
  if (view === 'wizard') return <ProjectCreationWizard onBack={() => setView('home')} onCreate={onCreate} onCreated={onCreated} onExit={onExit} />;
  return <main className="project-launch-screen">
    <div className="launch-drag-region titlebar-drag pywebview-drag-region" />
    <WindowButtons onExit={onExit} />
...
function ProjectCreationWizard({ onBack, onCreate, onCreated, onExit }: { onBack: () => void; onCreate: (options: ProjectCreationOptions) => Promise<Project>; onCreated: () => void; onExit: () => void }) {
  ...
  return <main className="project-launch-screen creation-mode">
    <div className="launch-drag-region titlebar-drag pywebview-drag-region" />
    <WindowButtons onExit={onExit} compact />
    <aside className="creation-sidebar">
      <button className="creation-back titlebar-no-drag" onClick={onBack}><ArrowLeft />返回启动页</button>
