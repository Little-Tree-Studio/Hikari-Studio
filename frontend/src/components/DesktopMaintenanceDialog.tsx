...
export function DesktopMaintenanceDialog({ open, close, notify, requestConfirm }: Props) {
  ...
  return <AnimatedModal open={open} close={close} className="maintenance-dialog" labelledBy="maintenance-title">
    ...
    {tab === 'updates' ? <div className="maintenance-body">
      <section className="maintenance-version"><div className="maintenance-logo">H</div><div><strong>{app?.name ?? 'Hikari Studio'}</strong><span>v{app?.version ?? updates?.currentVersion ?? '0.4.0-beta.1'} · {channel === 'beta' ? 'Beta 预览通道' : '稳定通道'}</span></div><label>更新通道<select value={channel} disabled={isBusy} onChange={(event) => setChannel(event.target.value as 'stable' | 'beta')}><option value="beta">Beta</option><option value="stable">Stable</option></select></label></section>
