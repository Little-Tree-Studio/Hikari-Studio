...
import { AlertTriangle, ArchiveRestore, Check, CheckCircle2, EyeOff, History, LoaderCircle, LocateFixed, Menu, MessageSquareText, Palette, Play, RotateCcw, Save, Settings2, X, Zap, type LucideIcon } from 'lucide-react';
...
interface GameUiThemeDialogProps {
  open: boolean;
  project: Project;
  close: () => void;
...
const FALLBACK_UI: NonNullable<Project['ui']> = { theme: 'hikari-light', dialogueStyle: 'glass' };
...
function RangeField({ label, value, min, max, step = 1, unit, onChange }: { label: string; value: number; min: number; max: number; step?: number; unit: string; onChange: (value: number) => void }) {
  return <label className="theme-range-field"><span>{label}<strong>{Number.isInteger(step) ? value : value.toFixed(2)}{unit}</strong></span><input type="range" min={min} max={max} step={step} value={value} onChange={(event) => onChange(Number(event.target.value))} /></label>;
}
...
export function GameUiThemeDialog({ open, project, close, apply, relinkAsset }: GameUiThemeDialogProps) {
  ...
  }, [open, project.assets.map((asset) => `${asset.id}:${asset.path}:${asset.uri ?? ''}:${asset.size ?? 0}`).join('|')]);
  ...
  if (!open) return null;
  return <div className="modal-backdrop theme-editor-backdrop" onClick={close}>
    <section className="modal game-theme-editor" role="dialog" aria-modal="true" aria-labelledby="theme-editor-title" onClick={(event) => event.stopPropagation()}>
      <header className="modal-header"><Palette /><div><strong id="theme-editor-title">游戏 UI 主题</strong><small>编辑器预览与导出游戏使用同一套主题参数</small></div><button className="icon-button" title="关闭" onClick={close}><X /></button></header>
      <div className="game-theme-workspace">
        <aside className="game-theme-controls">
          ...
          <section><div className="theme-section-heading"><strong>姓名样式</strong></div><div className="theme-segmented">{([['plain', '纯文字'], ['accent', '强调'], ['plate', '姓名牌']] as const).map(([value, label]) => <button type="button" className={theme.speakerStyle === value ? 'active' : ''} key={value} onClick={() => patchTheme({ speakerStyle: value })}>{label}</button>)}</div><ColorField label="姓名颜色" value={theme.speakerColor} onChange={(value) => patchTheme({ speakerColor: value })} /><RangeField label="姓名字号" value={theme.speakerFontSize} min={10} max={30} unit="px" onChange={(value) => patchTheme({ speakerFontSize: value })} /><label className="field"><span>字重</span><select value={theme.speakerWeight} onChange={(event) => patchTheme({ speakerWeight: Number(event.target.value) })}><option value="400">常规</option><option value="600">半粗</option><option value="700">粗体</option><option value="900">特粗</option></select></label></section>
          ...
          <section><div className="theme-section-heading"><strong>标题画面</strong></div><label className="field"><span>游戏版本</span><input value={gameVersion} onChange={(event) => setGameVersion(event.target.value)} placeholder="1.0.0" /></label><label className="field"><span>标题副标题</span><input value={title.subtitle ?? ''} onChange={(event) => setTitle({ ...title, subtitle: event.target.value })} placeholder={project.meta.author || 'Hikari Studio'} /></label><label className="field"><span>标题背景</span><select value={title.backgroundAssetId ?? ''} onChange={(event) => setTitle({ ...title, backgroundAssetId: event.target.value || undefined })}><option value="">自动使用第一个场景</option>{imageAssets.filter((asset) => asset.kind !== 'character').map((asset) => <option key={asset.id} value={asset.id}>{asset.name}</option>)}</select></label><AssetHealth assetId={backgroundAssetId} label={title.backgroundAssetId ? '标题背景' : '自动标题背景'} /><label className="field"><span>标题 Logo</span><select value={title.logoAssetId ?? ''} onChange={(event) => setTitle({ ...title, logoAssetId: event.target.value || undefined })}><option value="">使用项目名称文字</option>{imageAssets.map((asset) => <option key={asset.id} value={asset.id}>{asset.name}</option>)}</select></label><AssetHealth assetId={title.logoAssetId} label="标题 Logo" /></section>
        </aside>
        ...
        <div className="game-theme-preview-area">
          ...
          <div className="theme-preview-stage" style={previewStyle}>
            {theme.fontAssetId && project.assets.find((asset) => asset.id === theme.fontAssetId)?.uri && <style>{`@font-face{font-family:"Hikari Project Font";src:url(${JSON.stringify(project.assets.find((asset) => asset.id === theme.fontAssetId)?.uri)})}`}</style>}
      ...
      <footer className="modal-footer"><button className="button ghost" onClick={close}>取消</button><button className="button primary" onClick={() => apply({ ...(project.ui ?? FALLBACK_UI), title, runtimeTheme: normalizeGameUiTheme(theme) }, gameVersion.trim() || '1.0.0')}><Check />应用主题</button></footer>
    </section>
  </div>;
}
