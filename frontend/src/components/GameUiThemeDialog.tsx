    ...
    <section className="modal game-theme-editor" role="dialog" aria-modal="true" aria-labelledby="theme-editor-title" onClick={(event) => event.stopPropagation()}>
      ...
      <div className="game-theme-workspace">
        <aside className="game-theme-controls">
          ...
          <section><div className="theme-section-heading"><strong>标题画面</strong></div><label className="field"><span>游戏版本</span><input value={gameVersion} onChange={(event) => setGameVersion(event.target.value)} placeholder="1.0.0" /></label><label className="field"><span>标题副标题</span><input value={title.subtitle ?? ''} onChange={(event) => setTitle({ ...title, subtitle: event.target.value })} placeholder={project.meta.author || 'Hikari Studio'} /></label><label className="field"><span>标题背景</span><Select value={title.backgroundAssetId ?? ''} onChange={(value) => setTitle({ ...title, backgroundAssetId: value || undefined })}><option value="">自动使用第一个场景</option>{imageAssets.filter((asset) => asset.kind !== 'character').map((asset) => <option key={asset.id} value={asset.id}>{asset.name}</option>)}</Select></label><AssetHealth assetId={backgroundAssetId} label={title.backgroundAssetId ? '标题背景' : '自动标题背景'} /><label className="field"><span>标题 Logo</span><Select value={title.logoAssetId ?? ''} onChange={(value) => setTitle({ ...title, logoAssetId: value || undefined })}><option value="">使用项目名称文字</option>{imageAssets.map((asset) => <option key={asset.id} value={asset.id}>{asset.name}</option>)}</Select></label><AssetHealth assetId={title.logoAssetId} label="标题 Logo" /></section>
        </aside>
