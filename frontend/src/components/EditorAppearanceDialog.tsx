...
export function EditorAppearanceDialog({ open, close, openGameTheme }: Props) {
  ...
  return <AnimatedModal open={open} close={close} className="appearance-dialog" labelledBy="appearance-title">
    <header className="modal-header appearance-header"><div className="modal-heading-icon"><Palette /></div><div><strong id="appearance-title">编辑器外观</strong><small>实时生效，仅保存在此设备，不会写入游戏项目</small></div><button className="icon-button" title="关闭" onClick={close}><X /></button></header>
