...
export function RuntimeSettingsSection({ project, apply }: RuntimeSettingsSectionProps) {
  ...
  return <>
    <div className="settings-page-body settings-grid">
      <div className="field full"><label>游戏分辨率</label><Select value={`${resolution[0]}x${resolution[1]}`} onChange={(value) => { const [width, height] = value.split('x').map(Number); setResolution([width, height]); }}>{!['1024x768', '1280x800', '1280x720', '1366x768', '1600x900', '1920x1080'].includes(`${resolution[0]}x${resolution[1]}`) && <option value={`${resolution[0]}x${resolution[1]}`}>{resolution[0]} × {resolution[1]}</option>}<option value="1024x768">1024 × 768</option><option value="1280x800">1280 × 800</option><option value="1280x720">1280 × 720</option><option value="1366x768">1366 × 768</option><option value="1600x900">1600 × 900</option><option value="1920x1080">1920 × 1080</option></Select></div>
