...
export function DesktopMaintenanceSection({ notify, requestConfirm }: Props) {
  ...
  const runCheck = async () => {
    setBusy('check');
    try { const status = await checkForUpdates(true, channel); setUpdates(status); notify(status.status === 'available' ? `发现 Slide Studio ${status.manifest?.version}` : '当前已是最新版本'); }
  ...
  const runInstall = async (version?: string) => {
    const accepted = await requestConfirm({ title: version ? `安装 Slide Studio ${version}` : '安装已下载更新', message: '安装程序会关闭当前编辑器。请确认项目已经保存，安装完成后可重新打开。', confirmText: '启动安装' });
    if (!accepted) return;
