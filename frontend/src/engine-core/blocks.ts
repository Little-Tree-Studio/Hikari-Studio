...
export const blockRegistry: Record<BlockType, BlockDefinition> = {
  ...
  setVariable: define('setVariable', '设置变量', { variable: { type: 'string', required: true } }, (project) => ({ id: id(), type: 'setVariable', version: 1, variable: Object.keys(project.variables)[0] ?? '新变量', value: 0 })),
  condition: define('condition', '条件判断', { variable: { type: 'string', required: true }, operator: { type: 'enum', values: ['eq', 'neq', 'gt', 'gte', 'lt', 'lte'] } }, (project) => ({ id: id(), type: 'condition', version: 1, variable: Object.keys(project.variables)[0] ?? '新变量', operator: 'eq', compareValue: 0, trueTarget: project.activeFragmentId }), targetIssues),
