...
import {
  ...
  Sparkles, Trash2, Undo2, UserPlus, UserRound, Users, X, PanelBottom, PanelRight, PictureInPicture2, LoaderCircle, ExternalLink,
} from 'lucide-react';
import {
  ...
  applyAiPatch, loadCommandHistory, loadCommandHistoryStats, loadRecoverySnapshot, openProject, openProjectPath, openRecentProject, optimizeBlockText, previewClipboardScript, previewScriptImport, readClipboardText, replaceAssetFile, saveCommandHistory, saveProject, saveProjectAs, writeClipboardText,
} from './api';
...
import { parseJsonBlocks, parsePlainBlocks, parseRenpyBlocks, serializePlain, serializeRenpy } from './core/scriptTextCodec';
...
type Page = 'script' | 'stage' | 'texts' | 'assets' | 'audio' | 'map' | 'insights' | 'characters' | 'scenes' | 'history' | 'ai';
...
const fallbackProject: Project = {
  ...
  characters: [
    ...
    { id: 'makoto', name: '真琴', color: '#4a90c4', expressions: ['默认', '微笑', '平静', '害羞', '惊讶'] },
  ],
  chapters: [
    { id: 'start', name: '开始', entry: true, fragments: [{ id: 'opening', name: '片头' }] },
    { id: 'c1', name: '第一章 · 雾中的来信', fragments: [{ id: 'lake-meeting', name: '湖畔相遇' }, { id: 'pier-talk', name: '码头谈话' }, { id: 'old-school', name: '旧校舍' }] },
  ],
  ...
  scripts: {
    ...
    'lake-meeting': [
      ...
      { id: 'b2', type: 'sound', title: 'morning_pier.ogg', volume: .5, loop: true },
      { id: 'b3', type: 'narration', text: '清晨五点四十分，雾还没有散。旧码头的木板上凝着露水。' },
      { id: 'b4', type: 'dialogue', speaker: '真琴', text: '这么早？海屋七点才开门。', expression: '平静' },
      { id: 'b5', type: 'dialogue', speaker: '苏芮', text: '今晚有流星雨，汐见町下一次能看到，是十一年后。', expression: '犹豫' },
      { id: 'b6', type: 'setVariable', variable: '好感度', value: 1 },
      { id: 'b7', type: 'branch', title: '如何回应她的邀请？', options: [{ text: '答应今晚一起看流星雨', target: 'pier-talk' }, { text: '先问清楚当年为什么不辞而别', target: 'old-school' }] },
    ],
    'pier-talk': [{ id: 'pt1', type: 'dialogue', speaker: '苏芮', text: '好。……谢谢你没有追问。', expression: '平静' }],
    'old-school': [{ id: 'school1', type: 'narration', text: '两人沿着堤坝走到废弃的小学。铁门锈住了，推开时发出很长的一声。' }],
  },
  assets: [
...
interface StoryCardProps {
  ...
  optimizing: boolean;
  onOptimizeText: (index: number) => void;
  dragging: boolean;
...
const StoryCard = memo(function StoryCard({ index, project, block, selected, asset, voiceAsset, onSelect, onContextMenu, onChange, onMove, onDuplicate, onDelete, optimizing, onOptimizeText, dragging, listDragging, onPointerDown, onPointerMove, onPointerUp, onPointerCancel }: StoryCardProps) {
  ...
  && previous.optimizing === next.optimizing
  && previous.dragging === next.dragging
  && previous.listDragging === next.listDragging);

function Inspector({ project, block, update, dock, setDock, notify, openWindow }: { project: Project; block?: StoryBlock; update: (patch: StoryBlockPatch) => void; dock: InspectorDock; setDock: (dock: InspectorDock) => void; notify: (message: string, tone?: 'error' | 'success') => void; openWindow?: () => void }) {
  const header = <div className="inspector-header"><strong>属性检查器</strong>{block && <span>{blockMeta[block.type].name}</span>}<div className="inspector-dock-controls" role="group" aria-label="属性检查器停靠位置"><button className={dock === 'preview' ? 'active' : ''} title="停靠在预览下方" onClick={() => setDock('preview')}><PanelRight /></button><button className={dock === 'editor' ? 'active' : ''} title="停靠在编辑器下方" onClick={() => setDock('editor')}><PanelBottom /></button><button className={dock === 'floating' ? 'active' : ''} title="浮动面板" onClick={() => setDock('floating')}><PictureInPicture2 /></button>{openWindow && <button title="在新窗口打开属性检测器" onClick={openWindow}><ExternalLink /></button>}</div></div>;
...
function ScriptPage({ project, commit, selected, setSelected, view, setView, openBlocks, openImport, requestConfirm, openFragmentIds, activateFragment, closeFragment, closeOtherFragments, closeAllFragments, reorderFragmentTabs, inspectorDock, setInspectorDock, initialScrollTop, saveScrollTop, debugRunning, notify, pendingBlockReveal, completeBlockReveal, previewLanguage, onPreviewLanguageChange }: ScriptPageProps) {
  ...
  const optimizingIndexRef = useRef<number | null>(null);
  ...
  const optimizeText = async (index: number) => {
    ...
    if (!block || (block.type !== 'narration' && block.type !== 'dialogue')) return;
    if (optimizingIndexRef.current !== null) return;
    optimizingIndexRef.current = index;
    setOptimizingIndex(index);
    const startedAt = performance.now();
    const finish = () => {
      optimizingIndexRef.current = null;
      setOptimizingIndex(null);
    };
    try {
      const context = block.type === 'dialogue' ? { speaker: block.speaker, expression: block.expression } : {};
      const optimized = await optimizeBlockText(block.text ?? '', block.type, context);
      if (optimized && optimized !== block.text) updateBlock(index, { text: optimized });
      notify(optimized && optimized !== block.text ? '文本已由 AI 优化' : 'AI 返回的文本与原文本相同', 'success');
    } catch (error) {
      notify(error instanceof Error ? error.message : String(error), 'error');
    } finally {
      // 即使 AI 立即返回/失败，也让加载动画至少可见一小段时间，避免一闪而过。
      const remaining = 600 - (performance.now() - startedAt);
  ...
  const code = useMemo(() => serializeRenpy(blocks), [blocks]);
  const plainText = useMemo(() => serializePlain(blocks), [blocks]);
  const jsonText = useMemo(() => JSON.stringify(blocks, null, 2), [blocks]);
  const [textDraft, setTextDraft] = useState<string | null>(null);
  const blocksRef = useRef(blocks);
  blocksRef.current = blocks;
  useEffect(() => { setTextDraft(null); }, [view, project.activeFragmentId]);
  const derivedText = view === 'json' ? jsonText : view === 'code' ? code : plainText;
  const textValue = textDraft ?? derivedText;
  const commitTextView = (text: string) => {
    try {
      const next = view === 'json' ? parseJsonBlocks(text) : view === 'code' ? parseRenpyBlocks(text, blocksRef.current) : parsePlainBlocks(text, blocksRef.current);
      commit((current) => ({ ...current, scripts: { ...current.scripts, [current.activeFragmentId]: next } }));
    } catch { /* 输入尚未构成合法结构（例如 JSON 未闭合），保留草稿等待继续输入 */ }
  };
  useEffect(() => {
    if (textDraft === null) return;
    const timer = window.setTimeout(() => commitTextView(textDraft), 400);
  ...
  useEffect(() => {
    const receiveInspectorPatch = (event: MessageEvent) => {
      if (event.source !== window || event.data?.type !== 'slide-inspector-patch' || event.data.index !== selected) return;
      if (event.data.patch && typeof event.data.patch === 'object') updateBlock(selected, event.data.patch as StoryBlockPatch);
    };
    window.addEventListener('message', receiveInspectorPatch);
    return () => window.removeEventListener('message', receiveInspectorPatch);
  }, [selected, updateBlock]);
  const openInspectorWindow = () => {
    const popup = window.open('', `slide-inspector-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, 'popup,width=520,height=720');
    if (!popup) { notify('属性检测器窗口被系统拦截', 'error'); return; }
    const block = selectedBlock;
    popup.document.write(`<title>属性检测器</title><style>body{font:13px system-ui;margin:0;background:#f5f7f8;color:#18252b}header{padding:14px 16px;border-bottom:1px solid #d9e0e3;background:white;font-weight:700}main{padding:16px}textarea{width:100%;height:520px;box-sizing:border-box;font:12px ui-monospace,monospace;padding:10px;border:1px solid #cbd5d9;border-radius:6px}button{margin-top:12px;padding:9px 14px;border:0;border-radius:6px;background:#167c6b;color:white;cursor:pointer}</style><header>属性检测器 · ${block ? blockMeta[block.type].name : '未选择 Block'}</header><main><textarea id="props" ${block ? '' : 'disabled'}>${block ? JSON.stringify(block, null, 2).replace(/</g, '&lt;') : '请先在编辑器中选择一个 Block'}</textarea><button id="apply" ${block ? '' : 'disabled'}>应用到当前 Block</button></main>`);
    popup.document.close();
    popup.document.getElementById('apply')?.addEventListener('click', () => {
      try { const value = JSON.parse((popup.document.getElementById('props') as HTMLTextAreaElement).value); popup.opener?.postMessage({ type: 'slide-inspector-patch', index: selected, patch: value }, '*'); } catch { popup.alert('JSON 格式不正确'); }
    });
  };
  const inspector = <><Profiler id="inspector" onRender={recordComponentRender}><Inspector project={project} block={selectedBlock} update={(patch) => updateBlock(selected, patch)} dock={inspectorDock} setDock={setInspectorDock} notify={notify} openWindow={openInspectorWindow} /></Profiler>{droppedAssets.length > 0 && <EditorAssetImportDialog assets={droppedAssets} characters={project.characters} sourceLabel="剧本编辑器" close={() => setDroppedAssets([])} apply={applyDroppedAssets} />}</>;
  return <div className={`editor-layout inspector-${inspectorDock}`} data-selected-block-index={selected}><section className="editor-pane"><EditorTabBar openFragmentIds={openFragmentIds} activeFragmentId={project.activeFragmentId} fragmentNames={fragmentNames} activateFragment={activateFragment} closeFragment={closeFragment} closeOtherFragments={closeOtherFragments} closeAllFragments={closeAllFragments} reorderFragmentTabs={reorderFragmentTabs} /><div className="editor-toolbar"><div className="editor-title"><strong>{activeName}</strong><small>{blocks.length} Blocks</small></div><button className="button ghost" onClick={openImport}><FileUp /> 导入剧本</button><div className="view-switch">{([['cards', '卡片'], ['plain', '纯文本'], ['code', "Ren'Py"], ['json', 'JSON']] as [View, string][]).map(([key, name]) => <button key={key} className={`view-button ${view === key ? 'active' : ''}`} onClick={() => setView(key)}>{name}</button>)}</div></div>
    ...
    {view === 'code' && <textarea className="code-editor" value={textValue} onChange={(event) => setTextDraft(event.target.value)} spellCheck={false} aria-label="Ren'Py 编辑器" />}{view === 'json' && <textarea className="json-editor" value={textValue} onChange={(event) => setTextDraft(event.target.value)} spellCheck={false} aria-label="JSON 编辑器" />}{view === 'plain' && <textarea className="plain-text-editor" value={textValue} onChange={(event) => setTextDraft(event.target.value)} aria-label="纯文本编辑器" />}
